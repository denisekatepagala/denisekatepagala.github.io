...
def load_grades():
    ...
    if final_grade is not None:
        final_grade_label.configure(text=f"Final Grade: {final_grade:.2f}%")
    else:
        final_grade_label.configure(text="Final Grade:")
    ...
    for component in data["grades"]:
        ...
        #* FOR ADD_COMPONENT()
        component_left_text = CTkLabel(top_left_window_frame, text=component_str,
            font=("Courier New", 15))
        percentage_text = CTkLabel(top_left_window_frame, text=f"{percentage}%",
            font=("Courier New", 15))
        component_left_text.grid(row=component_row, column=0, padx=20, pady=5)
        percentage_text.grid(row=component_row, column=1, padx=20, pady=5)

        grade_right_window_frame.grid_columnconfigure(0, weight=1)
        grade_right_window_frame.grid_columnconfigure(1, weight=1)
        component_frame = CTkFrame(grade_right_window_frame, fg_color = "white", corner_radius = 8)
        component_index = len(grades)

        row = component_index // 2
        column = component_index % 2

        grade_right_window_frame.grid_rowconfigure(row, weight=1)

        component_frame.grid(row = row, column = column, padx=10, pady=10, sticky = 'nsew')
        CTkButton(component_frame, text = "➕ Add", width = 70, 
                fg_color = "#1D2128", text_color="#F4F2F2",
                corner_radius=10, font=('Roboto', 10),
                command=lambda: add_assessment(component_frame, component_str)).grid(row=9,column=0, padx=10, pady=10)
        CTkButton(component_frame, text = "➖ Delete", width = 70, 
                fg_color = "#1D2128", text_color="#F4F2F2",
                corner_radius=10, font=('Roboto', 10),
                command=lambda: delete_assessment(component_str)).grid(row=9, column=1, padx=10, pady=10)

        component_right_text = CTkLabel(component_frame, text=component_str, font=("Courier New", 15))
        component_right_text.grid(row=0,column=0, columnspan=2, padx=10, pady=10)

        saved_component = {
            "component": component_str,
            "percentage": percentage,
            "component_text" : component_left_text,
            "percentage_text" : percentage_text,
            "component_frame" : component_frame,
            "count" : 0,
            "assessments" : []
        }
        grades.append(saved_component)

        if "assessments" in component:
            for saved_assessment in component["assessments"]:
                saved_component["count"] += 1
                count = saved_component["count"]

                assessment = CTkLabel(component_frame, text = f"{component_str} {count}")
                score = CTkEntry(component_frame, width = 80, font=('Courier New', 15), 
                                placeholder_text="ex.: 10/10", placeholder_text_color = "grey")

                assessment.grid(row=count,column=0, padx=5, pady=10)
                score.grid(row=count, column=1, padx=5, pady=10)

                score.insert(0, saved_assessment["score"])

                saved_component["assessments"].append({
...
def add_assessment(component_frame, component_str):
    data = load_data()
    for component in grades:
        if component["component"] == component_str : 
            component["count"] += 1
            count = component["count"]
            
    
            assessment = CTkLabel(component_frame, text = f"{component_str} {count}", font = ('Courier New', 12))
            score = CTkEntry(component_frame, font=('Courier New', 15), 
                                placeholder_text="ex.: 10/10", placeholder_text_color = "grey")

            assessment.grid(row=count,column=0, padx=5, pady=3)
            score.grid(row=count, column=1, padx=5, pady=3, columnspan=2)

            component["assessments"].append({
                "name" : f"{component_str} {count}",
                "label" : assessment,
                "score" : score
            })


def delete_assessment(component_str):
    for component in grades:
        if component["component"] == component_str:
            if not component["assessments"]:
                return
            
            last = component["assessments"].pop()
    ...
    for saved_component in data["grades"]:
        if saved_component["component"] == component_str:
            if "assessments" in saved_component:
                saved_component["assessments"].pop()
            break
...
def compute_FG():
    ...
    for component in grades:
        ...
        for assessment in component["assessments"]:
            score = assessment["score"].get().strip()
            ...
            try:
                ...
                assessment_grade = (numerator / denominator) * 100

                total_score += assessment_grade
                total_items += 1
        ...
        if total_items > 0:
            component_grade = total_score / total_items
        else:
            component_grade = 0

        #* Computation
        weighted_grade = component_grade * (percentage / 100)
        final_grade += weighted_grade

        for saved_component in data["grades"]:
            if saved_component["component"] == component_str:
                saved_component["assessments"] = []
...
def delete_component():
    ...
    for saved_components in grades:
        if saved_components["component"] == component_str and saved_components["percentage"] == percentage:
            saved_components["component_text"].destroy()
            saved_components["percentage_text"].destroy()
            saved_components["component_frame"].destroy()

            grades.remove(saved_components)
            component_row -= 1
            break
        else:
            error_label_grade.configure(text= "Component Not Available!")


    data = load_data()

    for component in data["grades"]:
        if component["component"] == component_str and component["percentage"] == percentage:
            data["grades"].remove(component)
            break
...
#* Label Statistics
final_grade_label = CTkLabel(course_label_frame, text = "Final Grade: ",
                       font = ('Courier New', 20), text_color = "black")
final_grade_label.pack(side="left", padx = 45, pady = 15)

#* Content Frame/ Right Pannel
grade_right_window_frame = CTkScrollableFrame(course_border, height = 300,
                                                fg_color = "#F4F2F2", corner_radius = 8)
...
grade_left_window_frame = CTkFrame(course_border, height = 300,
                                   fg_color = "transparent")
grade_left_window_frame. pack(side = "left",fill = "y")
top_left_window_frame = CTkFrame(grade_left_window_frame, fg_color="#F4F2F2", corner_radius = 8, width = 80)
top_left_window_frame.pack(side="top", pady=(10,5), padx=(10,5), fill="both", expand = True)
bottom_left_window_frame = CTkFrame(grade_left_window_frame, fg_color="#F4F2F2", corner_radius = 8, width = 80)
bottom_left_window_frame.pack(side="bottom", pady=(5,10), padx=(10,5), fill="both")
...
component_entry.grid(row=0,column=1, padx=(10,20), pady=(20,10))
...
percentage_entry = CTkEntry(bottom_left_window_frame, placeholder_text = "ex.: 10",
                           width = 150, placeholder_text_color = "grey")
percentage_entry.grid(row=1,column=1, padx=(10,20), pady=(20,10))
...
CTkButton(bottom_left_window_frame, text = "Delete a Component", width = 130, 
          fg_color = "#1D2128", text_color="#F4F2F2",
          corner_radius=10, font=('Roboto', 12),
          command=delete_component).grid(row=3, column=1, padx=10, pady=10)
CTkButton(course_label_frame, text = "Compute Final Grade", width = 130, 
          fg_color = "#1D2128", text_color="#F4F2F2",
          corner_radius=10, font=('Roboto', 12),
          command=compute_FG).pack(side="right", padx = 45, pady = 15)
